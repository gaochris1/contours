...
var allContourLayer = new OpenLayers.Layer.Vector("AllContour", {
		styleMap: allContourStyle,					
		displayInLayerSwitcher:false
	});

function allContourHandler(contourRequest){
    var allContourP = new OpenLayers.Format.GeoJSON(options);
    allContourLayer.addFeatures(allContourP.read(contourRequest.responseText));
}

var contourRequest = OpenLayers.Request.GET({
    url:"data/contours.geojson",
    callback:allContourHandler
});



layers.push(allContourLayer);
...
var allSourceLayer = new OpenLayers.Layer.Vector("allSource", {
    styleMap: allSourceStyle,
    displayInLayerSwitcher: false
});

function allSourceHandler(request){
    var allSourceP = new OpenLayers.Format.GeoJSON(options);
    allSourceLayer.addFeatures(allSourceP.read(request.responseText));
}

var request = OpenLayers.Request.GET({
    url:"data/source.geojson",
    callback:allSourceHandler
});

layers.push(allSourceLayer);
map.addLayers(layers);
